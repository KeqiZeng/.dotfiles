...
zstyle ':fzf-tab:complete:*' fzf-preview 'if [[ -n $realpath ]]; then if [ -d $realpath ]; then echo $realpath/ && eza -1 --color always --icons $realpath | head -500; else if [[ $(file --mime $realpath) =~ binary ]]; then echo $realpath is a binary file; else bat --style=numbers --color=always --line-range :500 $realpath; fi; fi; fi'
setopt globdots

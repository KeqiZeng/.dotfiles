...
DOWN_FORMAT=""
if [ "$DOWN" -gt "999" ]; then
  DOWN_FORMAT=$(echo "$DOWN" | awk '{ printf "%03.0f Mb/s", $1 / 1000}')
else
  DOWN_FORMAT=$(echo "$DOWN" | awk '{ printf "%03.0f kb/s", $1}')
fi

UP_FORMAT=""
if [ "$UP" -gt "999" ]; then
  UP_FORMAT=$(echo "$UP" | awk '{ printf "%03.0f Mb/s", $1 / 1000}')
else
  UP_FORMAT=$(echo "$UP" | awk '{ printf "%03.0f kb/s", $1}')
fi

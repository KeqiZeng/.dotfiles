  ...
  while IFS= read -r device; do
    ...
    args+=(--add item volume.device.$COUNTER popup.volume_icon \
           --set volume.device.$COUNTER label="${device}" \
                                        label.color="$COLOR" \
                 click_script="SwitchAudioSource -s \"${device}\" && sketchybar --set /volume.device\.*/ label.color=$FOREGROUND --set \$NAME label.color=$BLUE --set volume_icon popup.drawing=off")
    COUNTER=$((COUNTER+1))
